...
jn1=${jn}_1
jid[1]=$(sbatch ${slurm_args} --job-name=${jn1} --export=SEED=${seed},LOAD_SUB='None' ${script} ${args} | tr -dc '0-9')
for j in {2..2}
do
jid[${j}]=$(sbatch ${slurm_args} --job-name=${jn}_${j} --dependency=afterok:${jid[$((j-1))]} --export=SEED=${seed},LOAD_SUB=${jn}_$((j-1))_${jid[$((j-1))]} ${script} ${args} | tr -dc '0-9')
done
}
